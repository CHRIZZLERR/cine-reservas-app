...
def seat_button(seat: dict) -> rx.Component:
    return rx.button(
        seat["id"],
        class_name=rx.cond(State.selected_seats.contains(seat["id"]), "seat seat-selected", rx.cond(seat["estado"] == "reservado", "seat seat-reserved", rx.cond(seat["tipo"] == "vip", "seat seat-vip", "seat"))),
...
def seat_map() -> rx.Component:
    return rx.box(
        ...
        rx.vstack(
            seat_row("A", State.row_A), seat_row("B", State.row_B), seat_row("C", State.row_C),
            seat_row("D", State.row_D), seat_row("E", State.row_E), seat_row("F", State.row_F),
            seat_row("G", State.row_G), seat_row("H", State.row_H), seat_row("I", State.row_I),
            seat_row("J", State.row_J), seat_row("K", State.row_K), seat_row("L", State.row_L),
            spacing="2", class_name="seat-map",
        ),
        rx.hstack(
            ...
            spacing="4", wrap="wrap", class_name="legend",
        ),
        class_name="seat-card",
    )