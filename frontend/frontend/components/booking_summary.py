...
def customer_form() -> rx.Component:
    return rx.box(
        rx.hstack(rx.box("👤", class_name="form-icon"), rx.vstack(rx.text("DATOS DE RESERVA", class_name="block-label"), rx.text("Completa tus datos para generar el código de reserva.", class_name="form-subtitle"), spacing="1", align="start"), align="center", spacing="3", class_name="form-header"),
        rx.grid(
            ...
            rx.box(rx.text("Método de pago", class_name="payment-label"), rx.text("Pago en taquilla", class_name="payment-value"), class_name="payment-card"),
...
def invoice() -> rx.Component:
    return rx.box(
        ...
        rx.hstack(rx.text(State.selected_date, class_name="invoice-muted"), rx.text("·", class_name="invoice-muted"), rx.text(State.selected_showtime, class_name="invoice-muted"), spacing="1"),
        ...
        rx.cond(State.booking_step == 1, rx.button("Siguiente: comida", class_name="checkout-btn", on_click=State.next_step),
            rx.cond(State.booking_step == 2, rx.button("Siguiente: datos", class_name="checkout-btn", on_click=State.next_step),
                rx.cond(State.booking_step == 3, rx.button("Confirmar reserva", class_name="checkout-btn", on_click=State.confirm_reservation),
