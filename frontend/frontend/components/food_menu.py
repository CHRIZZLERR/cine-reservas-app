...
def food_card(item: dict) -> rx.Component:
    return rx.box(
        rx.box(
            rx.text(item["nombre"], class_name="food-badge-title"),
            class_name="food-premium-img",
        ),
        rx.box(
        ...
        rx.hstack(
            rx.button(
                "−",
                class_name="qty-btn",
                on_click=lambda: State.remove_food(item["id"]),
            ),
            rx.text(item["qty"], class_name="qty-text"),
            rx.button(
                "+",
                class_name="qty-btn",
                on_click=lambda: State.add_food(item["id"]),
            ),
            spacing="2",
            align="center",
            class_name="qty-control",
        ),
        class_name="food-card food-card-premium",
    )
    ...
    return rx.box(
        rx.hstack(
            ...
            rx.vstack(
                ...
                rx.text(
                    "Agrega tus combos favoritos a la reserva.",
